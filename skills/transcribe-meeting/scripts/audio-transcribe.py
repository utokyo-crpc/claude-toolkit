...
def _record_usage(stage: str, model: str, resp) -> None:
...
def write_usage_report(out_dir: Path, stem: str):
    """stage 別・モデル別のトークン消費と概算コストを表示し、_usage.json に保存する。"""
    if not USAGE_LOG:
        return None
    by_key: dict = {}
    for u in USAGE_LOG:
        d = by_key.setdefault((u["stage"], u["model"]),
                              {"calls": 0, "prompt": 0, "candidates": 0, "total": 0})
        d["calls"] += 1
    ...
    for (stage, model), d in sorted(by_key.items()):
        ...
        est_cost += cost
        rows.append({"stage": stage, "model": model, **d, "est_usd": round(cost, 4)})
    report = {"total_tokens": total_tokens, "est_usd_approx": round(est_cost, 4),
              "note": "est_usd はテキスト単価による概算。音声入力の実請求とは異なる。",
              "by_stage": rows, "calls": USAGE_LOG}
    path = out_dir / f"{stem}_usage.json"
    path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print("\n── トークン消費（Gemini API） ──")
    for r in rows:
        print(f"  {r['stage']:<14}{r['model']:<20}calls={r['calls']:>2}  "
              f"total={r['total']:>9,}  (in={r['prompt']:,} / out={r['candidates']:,})  ~${r['est_usd']}")
    print(f"  合計 {total_tokens:,} トークン ／ 概算 ${round(est_cost, 4)}（{path.name} に保存）")
    return path
...
- 話者名は段落冒頭または文中で自然に示す。確度記号〔◎〕〔○〕〔△〕〔？〕が付いている場合は保持する（例：「古賀部長〔◎〕より：…」）
- 意味のない相槌のみの行は削除する
- 同じ内容を繰り返さない
- 出力はMarkdownのみ（説明文不要）
...
- 事実・数値・固有名詞・意思決定・話者名（確度記号付きなら保持）を落とさない
- 時系列と論理の流れを保つ
- 出力はMarkdownのみ（説明文不要）
...
def derive_files(client, transcript: str, out_dir: Path, stem: str) -> tuple:
    ...
    summary_out.write_text(collapse_loops(summary).strip() + "\n", encoding="utf-8")
